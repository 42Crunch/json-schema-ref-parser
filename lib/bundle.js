...
const {  MissingPointerError } = require("./util/errors");
...
function inventory$Ref ($refParent, $refKey, path, pathFromRoot, indirections, inventory, $refs, options) {
  ...
  if (pointer === null) {
    let err = new MissingPointerError("", path);
    err.path = url.safePointerToPath(url.getHash(path));
    err.path.push($refKey);
    err.message = null;
    $refs._root$Ref.addError(err);
    return;
  }
...
function remap (inventory, options) {
  ...
  for (let entry of inventory) {
    ...
    if (!entry.external) {
      ...
      const hook = options.hooks && options.hooks.onRemap;
      const pointer = hook ? hook(entry) : null;

      if (pointer) {
        // use pointer returned from the hook
        entry.$ref.$ref = pointer;
        pathFromRoot = pointer;
      } else {
